...
def get_sre_log_source():
    trace_id = lambda: f"tr-{uuid.uuid4().hex[:6]}"
    ...
    return {
        ...
        "IAM_PERMISSION_DENIED": {
            ...
            "logs": [
                ...
                f"[{get_timestamp()}] [ERROR] 403 Forbidden: service-account@project.iam.gserviceaccount.com does not have storage.objects.create access."
            ]
        },
        "MEMORY_LEAK_OOM": {
            "trace_id": trace_id(),
        ...
        "REDIS_CONNECTION_REFUSED": {
            ...
            "logs": [
                ...
                f"[{get_timestamp()}] [ERROR] redis.exceptions.ConnectionError: Error 111 connecting to localhost:6379. Connection refused."
            ]
        },
        "JWT_EXPIRED": {
            "trace_id": trace_id(),
