...
class SREAgentService:
    ...
    async def analyze_and_report_ui(self, error: str, trace_id: str, service_name: str, session_id: str = None):
        async def event_generator():
            full_response = ""
            user_id = "poc"

            if not session_id:
                session = await self.runner.session_service.create_session(user_id=user_id,
                                                                           app_name='sre_triage_root_agent')
            else:
                session = await self.runner.session_service.get_session(session_id=session_id, user_id=user_id,
                                                                        app_name='sre_triage_root_agent')
            session_input_message = f"""
            [INCOMING SIGNAL DETECTED]
            -------------------------
